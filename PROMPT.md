...
2. **📅 Next Week** — Plan the upcoming week's roster. Auto-fills from the scheduled group, but any slot can be swapped to a player from the other group or sub pool via dropdowns. Players already selected are disabled in other slots. A **"✏️ Type a name..."** option allows adding **fill-in players** not in any group — useful when someone is sick and a friend fills in. Fill-ins appear with a "FILL-IN" badge and orange highlight, and are tracked on the leaderboard but don't pollute the regular group rosters. Shows a playlist selector for the week. Requires exactly 8 players to generate. "Confirm & Generate Matches" button locks in the roster and creates match pairings.
